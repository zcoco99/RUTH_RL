...
class AtariEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(
        ...
        if self.game == 'seaquest': 
            self._action_set = [0, 2,3,4,5, 6,7,8,9] 
            self._action_steps = 4
        if self.game == 'montezuma_revenge':
            self._action_set = [0, 1, 5, 4, 3, 12, 14] 
            self._action_steps = 4
        if self.game == 'berzerk':
            self._action_set = [0, 1, 2, 3, 4, 5, ]
            self._action_steps = 40
        self.action_space = spaces.Box(low=np.array([-1]*len(self._action_set)), high=np.array([1]*len(self._action_set)), dtype=np.float32)
    ...
    def step(self, a):
        ...
        num_steps = self.np_random.randint( int(self._action_steps-4), self._action_steps+2)
    ...
    def _get_obs(self):
        ...
        #     img = self._get_image()
        # return img
        return [self._get_ram()/255.0, self._get_image()]
