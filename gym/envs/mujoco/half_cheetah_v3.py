...
class HalfCheetahEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        forward_reward = (forward_reward > 0.1) * 2.0
        angle_reward = 0.5 * np.cos(self.data.qpos[2])
        
        observation = self._get_obs()
        reward = forward_reward  + angle_reward
...
#        observation = np.concatenate((position, velocity)).ravel()
        ...
        # img = self.render_frame(camera_id=0, imagesize=(50,50)).copy()
