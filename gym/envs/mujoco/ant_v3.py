...
#        forward_reward = 0.5 - abs(np.sqrt(x_velocity**2+y_velocity**2) - 1) # x_velocity
        forward_reward = np.sqrt(x_velocity**2+y_velocity**2)
...
#        forward_reward, ctrl_cost, contact_cost, healthy_reward, forward_reward = 0,0,0,0,0
        reward = xy_velocity[0]
        reward = -np.sqrt(((xy_position_after-[3,3])**2).sum())/3 + self.healthy_reward
