...
class MujocoEnv(gym.Env):
    ...
    def __init__(self, model_path, frame_skip, rgb_rendering_tracking=True):
        ...
        assert not isinstance(observation, list)
        self._set_observation_space(observation)
        self.observation_space_disc = None
...
#        GlfwContext(offscreen=True)  # Create a window to init GLFW.
        ...
        if self.viewer is None:
            if mode == 'human':
                ...
                self.viewer = mujoco_py.MjRenderContextOffscreen(self.sim, offscreen=True, device_id=-1)
...
#            if self.rgb_rendering_tracking and camera_name in self.model.camera_names:
#                camera_id = self.model.camera_name2id(camera_name)
            _ = self._get_viewer(mode).render(width, height, camera_id=camera_id)
            # window size used for old mujoco-py:
            data = self._get_viewer(mode).read_pixels(width, height, depth=False)
            # original image is upside-down, so flip it
            return data[::-1, :, :]
